...
class SVGGenerator:
    def __init__(self, output_folder="generated_svgs", font_size=15, padding_scale=0.2):
    ...
    def _calculate_text_bounds(self, text, font_size, slant_value, width_value):
        """
        Dynamically calculate the bounding box of the text based on slant and width adjustments.
        Returns the bounds as (left_offset, right_offset, text_width, text_height).
        """
        text_length = len(text)

        # Adjust text proportions
        adjusted_width_factor = max(0.5, 1 + width_value / 100)  # Ensure width factor is not negative
        adjusted_font_size = font_size * adjusted_width_factor

        # Calculate base width and height
        text_width = text_length * adjusted_font_size // 2
        text_height = font_size

        # Calculate slant offsets
        slant_factor = abs(slant_value) / 60  # Proportion of slant (max 60 degrees)
        slant_offset = text_height * slant_factor

        left_offset = slant_offset if slant_value < 0 else 0
        right_offset = slant_offset if slant_value > 0 else 0

        return left_offset, right_offset, text_width, text_height
    ...
    def generate(self, weight_value=400, slant_value=0, width_value=0, text="AB ab"):
        ...
        """
        # Dynamically calculate text bounds
        left_offset, right_offset, text_width, text_height = self._calculate_text_bounds(
            text, self.font_size, slant_value, width_value
        )

        # Dynamically determine canvas size
        dynamic_padding = self.font_size * self.padding_scale  # Base padding relative to font size
        canvas_width = text_width + left_offset + right_offset + 2 * dynamic_padding
        canvas_height = text_height + 2 * dynamic_padding

        # Calculate text starting position to center it
        x_position = left_offset + dynamic_padding
        y_position = dynamic_padding + text_height
...
# # class SVGGenerator:
# #     def __init__(self, output_folder="generated_svgs", font_size=50, padding_scale=1, extra_right_padding_scale=0.5):
# #         self.output_folder = output_folder
# #         self.font_size = font_size  # Base font size
# #         self.padding_scale = padding_scale  # Padding scale relative to font size
...
# #         slant_padding_right = self.font_size * slant_offset
# #         slant_padding_left = max(0, self.font_size * slant_offset if slant_value < 0 else 0)
...
# #         width = (
# #             text_length * adjusted_font_size // 2
# #             + 2 * dynamic_padding
# #             + extra_right_padding
# #             + slant_padding_right
# #             + slant_padding_left
...
# #         height = self.font_size + 2 * dynamic_padding + slant_padding_right
...
# #         # Add text element to SVG
# #         x_position = dynamic_padding + slant_padding_left
...
# class SVGGenerator:
#     def __init__(self, output_folder="generated_svgs", font_size= 20, padding_scale=0.2, extra_right_padding_scale=0.4, min_left_spacing=40):
#         self.output_folder = output_folder
#         self.font_size = font_size  # Base font size
#         self.padding_scale = padding_scale  # Padding scale relative to font size
#         self.extra_right_padding_scale = extra_right_padding_scale  # Reduced additional right padding scale
#         self.min_left_spacing = min_left_spacing  # Minimum spacing from the left edge
...
#         slant_offset = abs(slant_value) / 60  # Calculate slant effect (max 60 degrees)
#         slant_padding_right = self.font_size * slant_offset
#         slant_padding_left = max(self.min_left_spacing, self.font_size * slant_offset if slant_value < 0 else self.min_left_spacing)

#         # Dynamically determine canvas size and padding
#         dynamic_padding = self.font_size * self.padding_scale  # Base padding relative to font size
#         extra_right_padding = self.font_size * self.extra_right_padding_scale  # Reduced extra padding for the right
#         text_length = len(text)
...
#         # Add text element to SVG
#         x_position = slant_padding_left + dynamic_padding
#         y_position = dynamic_padding + self.font_size
